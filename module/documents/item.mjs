...
export class AndragathimaItem extends Item {
  ...
  _prepareWeaponData(itemData) {
    ...
    if (this.actor) {
      ...
      if (isRanged) {
        data.totalAttack = (data.attack?.bonus || 0) + actorData.combat.ranged.attack;
      } else {
        data.totalAttack = (data.attack?.bonus || 0) + actorData.combat.melee.attack;
      }
  ...
  _prepareSpellData(itemData) {
    ...
    // Calculate spell power if actor is available
    if (this.actor) {
      const actorData = this.actor.system;
